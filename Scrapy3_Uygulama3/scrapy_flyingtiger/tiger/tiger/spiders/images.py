...
class ImagesSpider(scrapy.Spider):
    ...
    def parse_items(self, response):
        ...
        try:
            image_item['image_url'] = 'https:' + response.css('div.product__media img').attrib['src'].split('?')[0]
        except:
            image_item[
                'image_url'] = 'https://t3.ftcdn.net/jpg/04/62/93/66/360_F_462936689_BpEEcxfgMuYPfTaIAOC1tCDurmsno7Sp.jpg'

        image_item['product_code'] = response.css('div.product__sku::text').get().strip().split(':')[-1].strip()
